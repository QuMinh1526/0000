...
public class Aura extends Module {
	...
	public final Setting<SettingGroup> espGroup = new Setting<>("ESPSettings", new SettingGroup(false, 0), v -> esp.getValue() == ESP.NextGen || esp.getValue() == ESP.Dimasik || esp.getValue() == ESP.NextGen);
	...
	public void attack() {
		...
		int previousSlot = -1;
		if (usedMaceSwap) {
			previousSlot = ModuleManager.maceSwap.silentSwapToMace();
		} else {
			previousSlot = this.switchMethod();
		}
		...
		if (usedMaceSwap) {
			...
			ModuleManager.maceSwap.handleSwitchBackManual();
		} else {
			if (previousSlot != -1) {
